...
Install_Openssl3_010() {
    ...
    if [ -z "${openssl3Check}" ]; then
        if [ ! -f "/usr/local/openssl3010/bin/openssl" ]; then
            cd ${run_path}
            ...
            WITH_SSL="-DWITH_SSL=/usr/local/openssl3010"
        else
            WITH_SSL="-DWITH_SSL=/usr/local/openssl3010"
        fi
    else
...
EOF
    ...
    [ "${version}" == "5.7" ] || [[ "${version}" =~ ^8\. ]] && sed -i '/#log_queries_not_using_indexes/a\early-plugin-load = ""' /etc/my.cnf
    [ "${version}" == "5.6" ] || [ "${version}" == "5.7" ] || [[ "${version}" =~ ^8\. ]] && sed -i '/#skip-name-resolve/i\explicit_defaults_for_timestamp = true' /etc/my.cnf
...
Download_Src() {
    ...
    if [[ "${version}" =~ ^8\. ]] && [ "${actionType}" == "update" ]; then
        echo "======================================================================="
...
Install_Configure() {
    ...
    if [ "${version}" == "5.1" ]; then
        ...
        mkdir install
        cd install
        cmake .. -DCMAKE_INSTALL_PREFIX=${Setup_Path} -DSYSCONFDIR=/etc -DWITH_MYISAM_STORAGE_ENGINE=1 -DWITH_INNOBASE_STORAGE_ENGINE=1 -DWITH_PARTITION_STORAGE_ENGINE=1 -DWITH_FEDERATED_STORAGE_ENGINE=1 -DEXTRA_CHARSETS=all -DDEFAULT_CHARSET=utf8mb4 -DDEFAULT_COLLATION=utf8mb4_general_ci -DWITH_EMBEDDED_SERVER=1 -DENABLED_LOCAL_INFILE=1 -DWITH_BOOST=../boost ${WITH_SSL} ${i_make_args} ${GREATSQL_57_SET}
    elif [[ "${version}" =~ ^8\. ]]; then
        mkdir install
        cd install
        cmakeV="cmake"
        ...
        ${cmakeV} .. -DCMAKE_INSTALL_PREFIX=${Setup_Path} -DSYSCONFDIR=/etc -DWITH_MYISAM_STORAGE_ENGINE=1 -DWITH_INNOBASE_STORAGE_ENGINE=1 -DWITH_PARTITION_STORAGE_ENGINE=1 -DDEFAULT_CHARSET=utf8mb4 -DDEFAULT_COLLATION=utf8mb4_general_ci -DENABLED_LOCAL_INFILE=1 -DWITH_BOOST=../boost ${GREATSQL_80_SET} ${WITH_SSL} ${i_make_args}
...
Install_Mysql() {
    ...
    [[ "${version}" =~ ^8\. ]] || [ "${version}" == "mariadb_10.2" ] || [ "${version}" == "mariadb_10.3" ] || [ "${version}" == "mariadb_10.4" ] && echo "True" >${Setup_Path}/mysqlDb3.pl
}
Mysql_Initialize() {
    if [ -d "${Data_Path}" ]; then
        rm -rf ${Data_Path}/*
    ...
    elif [ "${version}" == "5.7" ] || [[ "${version}" =~ ^8\. ]]; then
...
if [ "${actionType}" == 'install' ] || [ "${actionType}" == "update" ]; then
    ...
    OPENSSL_30_VER=$(openssl version | grep -Eow "OpenSSL 3" | head -1)
    if [ "${version}" == '8.1' ]; then
        if [ "${OPENSSL_30_VER}" ]; then
            return
        else
            if [ "${PM}" == "yum" ] || [ "${PM}" == "dnf" ]; then
                ${PM} install perl perl-devel perl-Data-Dumper perl-IPC-Cmd lksctp-tools lksctp-tools lksctp-tools-devel libssh2-devel -y
            elif [ "${PM}" == "apt-get" ]; then
                ${PM} update
                ${PM} install perl libsctp-dev lksctp-tools zlib1g-dev libssh2-1-dev -y
            fi
            Install_Openssl3_010
        fi
    else
