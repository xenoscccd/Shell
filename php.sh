...
Install_Openssl3_010() {
	openssl3Check=$(openssl version | grep -Eow "OpenSSL 3" | head -1)
	if [ ! -f "/usr/local/openssl3010/bin/openssl" ] && [ -z "${openssl3Check}" ]; then
		cd ${run_path}
		wget ${download_Url}/src/openssl-${openssl3010Version}.tar.gz -T 20
		tar -zxf openssl-${openssl3010Version}.tar.gz
		rm -f openssl-${openssl3010Version}.tar.gz
		cd openssl-${openssl3010Version}
		./config --prefix=/usr/local/openssl3010 --openssldir=/usr/local/openssl3010 enable-md2 enable-rc5 sctp zlib-dynamic shared -fPIC
		make -j${cpuCore}
		make install
		[ $? -ne 0 ] && Error_Msg
		echo "/usr/local/openssl3010/lib64" >>/etc/ld.so.conf.d/zopenssl3010.conf
		ldconfig
		cd ..
		rm -rf openssl-${openssl3010Version}
	fi
}
Install_Curl() {
	if [ "${PM}" == "yum" ]; then
...
Install_Curl_New() {
	if [ "${php_version}" -ge "83" ]; then
		curlVersion="8.2.1"
		if [ ! -f "/usr/local/openssl3010/bin/openssl" ] && [ "${openssl3Check}" ]; then
			withSSL="--with-ssl"
		else
			withSSL="--with-ssl=/usr/local/openssl3010"
		fi
		withprefix="--prefix=/usr/local/curl_3"
		enableladp=" "
	else
		curlVersion="7.74.0"
		withSSL="--with-ssl=/usr/local/openssl111"
		withprefix="--prefix=/usr/local/curl_2"
		enableladp="--enable-ldap --enable-ldaps"
	fi

	wget ${download_Url}/src/curl-${curlVersion}.tar.gz
	tar -zxf curl-${curlVersion}.tar.gz
	cd curl-${curlVersion}
	rm -rf /usr/local/curl_2 /usr/local/curl_3
	./configure ${withprefix} ${enableladp} --with-brotli --with-libssh2 --with-libssh --enable-ares --with-gssapi --without-nss --enable-smb --with-libidn2 ${withSSL}
	[ $? -ne 0 ] && Error_Msg
	make -j${cpuCore}
	make install
	cd ..
...
Install_Configure() {
	...
	if [ "${php_version}" -ge "80" ]; then
		if [ "${php_version}" -ge "83" ]; then
			openssl3Check=$(openssl version | grep -Eow "OpenSSL 3" | head -1)
			if [ "${openssl3Check}" ]; then
				CURL_OPENSSL_LIB_VERSION="$(curl -V 2>&1 | grep -Eow 'OpenSSL/[0-9.]*' | awk -F '/' '{print $2}')"
				OPENSSL_LIB_VERSION="$(openssl version | awk '{print $2}')"
				if [ "${CURL_OPENSSL_LIB_VERSION}" != "${OPENSSL_LIB_VERSION}" ]; then
					Install_Curl_New
					withOpenssl=""
					withCurl="/usr/local/curl_3"
					export LDFLAGS=" -L${withCurl}/lib"
					export CFLAGS="-I${withCurl}/include"
					export LIBS="-L${withCurl}/lib"
				else
					withOpenssl=""
					withCurl=""
				fi
				...
				if [ "${PM}" == "yum" ] || [ "${PM}" == "dnf" ]; then
					${PM} install perl perl-devel perl-Data-Dumper perl-IPC-Cmd lksctp-tools lksctp-tools lksctp-tools-devel brotli-devel libssh2-devel -y
				elif [ "${PM}" == "apt-get" ]; then
					${PM} update
					${PM} install perl libsctp-dev lksctp-tools zlib1g-dev libbrotli-dev libssh2-1-dev -y
				fi
				Install_Openssl3_010
				Install_Curl_New
				withOpenssl="/usr/local/openssl3010"
				withCurl="/usr/local/curl_3"
			fi
		else
			...
			if [ -z "${opensslCheck}" ]; then
				if [ "${PM}" == "yum" ] || [ "${PM}" == "dnf" ]; then
					${PM} install lksctp-tools-devel brotli-devel libssh2-devel -y
				elif [ "${PM}" == "apt-get" ]; then
					${PM} install libsctp-dev libbrotli-dev libssh2-1-dev -y
				fi
				Install_Openssl_1_1_1
				Install_Curl_New
				withOpenssl="/usr/local/openssl111"
		...
		if [ "${php_version}" == "53" ]; then
			...
			if [ "${withOpenssl}" ]; then
				if [ "${php_version}" -ge "83" ]; then
					export LDFLAGS="-L${withOpenssl}/lib64 -L${withCurl}/lib"
					export CFLAGS="-I${withOpenssl}/include -I${withCurl}/include"
					export LIBS="-L${withOpenssl}/lib64 -L${withCurl}/lib -lcares -lssh2 -lssh2 -lssl -lcrypto -lssl -lcrypto -lgssapi_krb5 -lbrotlidec -lbrotlidec -lz"
				else
					export CFLAGS="-I${withOpenssl}/include -I${withCurl}/include"
...
Install_Zip_ext() {
	...
	if [ "${php_version}" == "73" ]; then
		...
		extFile="/www/server/php/83/lib/php/extensions/no-debug-non-zts-20220830/zip.so"
	fi
