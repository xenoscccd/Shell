...
Install_LuaJIT() {
    if [[ "${version}" =~ ^1\.2[345]$ ]] || [ "${version}" == "tengine" ]; then
        Install_LuaJIT2
        return
    fi
...
build_boringSSL() {
    ...
    if [ "${PM}" = "yum" ]; then
        if [ "${Centos7Check}" ]; then
            ...
            yum install libunwind-devel libunwind cmake3 make libarchive -y
            export CC=/opt/rh/devtoolset-9/root/usr/bin/gcc
            export CXX=/opt/rh/devtoolset-9/root/usr/bin/g++
            cmakeV="cmake3"
        else
            yum install libunwind-devel libunwind cmake3 make gcc gcc-c++ libarchive -y
            export CC=/usr/bin/gcc
            export CXX=/usr/bin/g++
        fi
    elif [ "${PM}" = "apt-get" ]; then
        apt-get update
        apt install build-essential ca-certificates zlib1g-dev libpcre3 libpcre3-dev libssl-dev git cmake mercurial libunwind-dev pkg-config -y
    fi
    #准备ninja编译工具
    wget --no-check-certificate ${download_Url}/src/ninja-linux.zip
    ...
    ARM_Check= $(uname -a | grep -E 'aarch64|arm|ARM')
    if [ "${ARM_Check}" ]; then
        wget --no-check-certificate https://dl.google.com/go/go1.21.0.linux-arm64.tar.gz
        tar -C $Setup_Path -xzf go1.21.0.linux-arm64.tar.gz
        export GOPATH="$HOME/go"
        export PATH="$PATH:$Setup_Path/go/bin:$GOPATH/bin"
    else
        wget --no-check-certificate https://dl.google.com/go/go1.21.0.linux-amd64.tar.gz
        tar -C $Setup_Path -xzf go1.21.0.linux-amd64.tar.gz
        export GOPATH="$HOME/go"
        export PATH="$PATH:$Setup_Path/go/bin:$GOPATH/bin"
    fi
    ...
    #下载boringssl
    wget --no-check-certificate ${download_Url}/src/boringssl.zip
    unzip boringssl.zip >/dev/null
    cd /www/server/nginx/src/boringssl/build/
...
Install_Configure() {
    ...
    if [ "${version}" == "1.25" ]; then
        ./configure --user=www --group=www --prefix=${Setup_Path} ${ENABLE_LUA} --add-module=${Setup_Path}/src/ngx_cache_purge ${ENABLE_STICKY} --with-pcre=pcre-${pcre_version} ${ENABLE_HTTP2} ${ENABLE_HTTP3} --with-http_stub_status_module --with-http_ssl_module --with-http_image_filter_module --with-http_gzip_static_module --with-http_gunzip_module --with-ipv6 --with-http_sub_module --with-http_flv_module --with-http_addition_module --with-http_realip_module --with-http_mp4_module --add-module=${Setup_Path}/src/ngx_http_substitutions_filter_module-master --with-cc-opt='-I./boringssl/include' --with-ld-opt="-L./boringssl/build/ssl -L./boringssl/build/crypto ${jemallocLD}" ${ENABLE_WEBDAV} ${ENABLE_NGX_PAGESPEED} ${ADD_EXTENSION} ${i_make_args}
    else
        ./configure --user=www --group=www --prefix=${Setup_Path} ${ENABLE_LUA} --add-module=${Setup_Path}/src/ngx_cache_purge ${ENABLE_STICKY} --with-openssl=${Setup_Path}/src/openssl --with-pcre=pcre-${pcre_version} ${ENABLE_HTTP2} --with-http_stub_status_module --with-http_ssl_module --with-http_image_filter_module --with-http_gzip_static_module --with-http_gunzip_module --with-ipv6 --with-http_sub_module --with-http_flv_module --with-http_addition_module --with-http_realip_module --with-http_mp4_module --add-module=${Setup_Path}/src/ngx_http_substitutions_filter_module-master --with-ld-opt="-Wl,-E ${jemallocLD}" --with-cc-opt="-Wno-error" ${ENABLE_WEBDAV} ${ENABLE_NGX_PAGESPEED} ${ADD_EXTENSION} ${i_make_args}
    fi
    make -j${cpuCore}
}
Install_Nginx() {
...
Set_Version() {
    if [ "${version}" == "tengine" ]; then
        echo "Tengine(${tengine})" >${Setup_Path}/version.pl
        echo "Tengine(${tengine})" >${Setup_Path}/version_check.pl
